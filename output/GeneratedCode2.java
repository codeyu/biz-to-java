/**
 * Generated code from C:\github\biz-to-java\input\t2.txt
 */

if(!StrUtil.eq(test222.getTestField9(), testTable333.getTestField9())) {
    test222.setTestField9(testTable333.getTestField9());
    this.DSSKK = "";
    this.IN20 = true;
    testTable333.setTestField7(0);
}

if(StrUtil.eq(this.DAABB, this.DSSKK) || StrUtil.eq(test222.getTestField9(), "")) {
    this.DAABB = "";
    this.WBBCC = "";
}

if(StrUtil.eq(this.DCC23, this.DUUHH) && StrUtil.eq(testTable333.getTestField9(), "")) {
    this.DMMNN = "";
    //TODO: 項目「F23(Ｌ０３(L11)TTT).(常務コ)」にブランクを代入します。
}

//TODO: if([項目「F23(Ｌ０３(L11)TTT).(常務コ)」 ＝ ﾌﾞﾗﾝｸ] && [項目「F23(Ｌ０３(L11)TTT).(常ド)」 ＝ ﾌﾞﾗﾝｸ]) {
//TODO:     this.DMMNN = "";
//TODO: }

if(!NumUtil.eq(test222.getTestField5(), testTable333.getTestField8())) {
    test222.setTestField9(testTable333.getTestField9());
}

